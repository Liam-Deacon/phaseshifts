...
def main():
    ...
    atorb_parser = subparsers.add_parser(
        "atorb",
        help="Generate atomic orbital input files for all known elements"
    )
    atorb_parser.add_argument(
        "--output-dir", type=str, default="./atorb_lib",
        help="Directory to write all generated atorb input files"
    )
    atorb_parser.add_argument(
        "--rel", action="store_true", default=False,
        help="Use relativistic calculation (default: False)"
    )
    atorb_parser.add_argument(
        "--ngrid", type=int, default=1000,
        help="Number of grid points (default: 1000)"
    )
    atorb_parser.add_argument(
        "--method", type=str, default="HF",
