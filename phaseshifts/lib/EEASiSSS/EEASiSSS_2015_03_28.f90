! program EEASiSSS  !version 2015_03_28
...
!==============================================================================
 ...
 !
 !I/O items.
 character(len=255) :: outdir,atdir
 !
 !basic constants.
 !alfa=fine-structure constant, c=speed of light, cm2=1/c**2.
 real(dp) :: pi
 real(dp),parameter :: rydb=13.60569172d0,bohr=0.5291772083d0,&
...
!radfac=factor in Mattheiss prescription (in superposition),
 ...
 character(len=7),allocatable :: id_at(:)
 real(dp),parameter :: radfac=0.98d0
...
real(dp),parameter :: wave_start=1.d-12
real(dp),parameter :: eta_cutoff=1.d-10
real(dp),parameter :: relerr0=1.d-02,relerr1=1.d-08,relerr2=1.d-08
end module param
...
contains
  ...
  real(dp), dimension(Dim_XC), intent(out) :: bestmem_XC
  !
  ...
  real(dp)  :: bj(0:lmax),x,xi,xl,x2h,x2hh,cl,f,s,almax
  ...
  implicit none
  real(dp) :: integral_egt
  integer,intent(in) :: n,m
  real(dp),intent(in) :: x1,x2,r(n),d,f(n)
  integer :: i1,i2,i
  real(dp) :: f1,f2,p1,p2,s,w1,w2
  ...
  implicit none
  real(dp) :: interpol_egl3
  integer,intent(in) :: n
  real(dp),intent(in) :: x,r(n),d,f(n)
  ...
  real(dp) :: simpson_s,rx(nx),dx,FUNCT(nx)
  ...
  implicit none
  integer :: nx,j
  real(dp) :: rx(nx),rho4pir2(nx),QvsR(nx),VvsR(nx),f(nx),s(nx)
  real(dp) :: z,dx,a
  do j=1,nx
...
real(dp),allocatable :: rx(:,:),dx(:),rmt(:),rmt0(:),&
  rmtn(:),rmtx(:),rmt1(:,:)
...
real(dp),allocatable :: ad(:,:),rk(:,:)
real(dp) :: rcX(3,3),volUC,volXC
end module space
...
implicit none
real(dp) :: V0,mVstp,rsis
real(dp),allocatable,dimension(:) :: z,cls,Vstp,Vrmt,V01
real(dp),allocatable,dimension(:,:) :: Vtot,VC,VCit,Vxc,QvsR,&
  atrho4pir2,myxc,rs
real(dp),allocatable :: Vtot1(:,:,:)
real(dp) :: E_DE
!
!Madelung items.
...
integer :: ne,ie
real(dp)  :: ee1,ee2,dee
real(dp),allocatable :: eev(:)
real(dp),parameter :: de1=1.d0 !(in eV)
real(dp)  :: rMTweight,dVeV,rerr,verr
!
!Vxc items.
!v0ev  = energy grid of V0.
!psave = parameter in xc_myxc, MTxc, and V0interstitial.
real(dp),allocatable :: v0ev(:)
real(dp) :: psave
!
...
real(dp),allocatable,dimension(:,:) :: sdat
...
contains
  ...
  integer :: i,j,k,iw,h1(nieq)
  real(dp) :: y00,beta,xs,myx,zz(nieq),r1(nieq),a1(nieq)
  real(dp),allocatable :: rho4pir2(:,:),a2(:,:)
  real(dp),parameter :: thrd=1.d0/3.d0
  Y00=1.d0/sqrt(4.d0*pi)

  fclmsum=trim(W2kData)//'.clmsum_ASA'
  write(61,'(a)') trim(fclmsum)
  fvcoul=trim(W2kData)//'.vcoul_ASA'
  write(61,'(a)') trim(fvcoul)
  !CHECK no. atoms.
  open(50,file=trim(fclmsum),status='old')
  iw=0
  do
    read(50,'(a)',end=1) txt
...
1 close(50)
  ...
  open(50,file=trim(fvcoul),status='old')
  do i=1,nieq
    do
      read(50,'(a)') txt
      if(txt(4:7)=='ATOM') exit
  ...
  do i=1,nieq
    ...
    rmt(i)=rx(nx(i),i)
    !cls.
    if(abs(cls(i))>1.d-04)then
      VC(1:nx(i),i)=VC(1:nx(i),i)+cls(i)/rydb !cls(eV), VC(Ry)
    endif
    !write-outs.
    if(RhoPot(1:1)=='y')then
      open(10,file=&
      trim(outdir)//trim(compound)//'.RHO4pir2.'//id_at(i),status='replace')
      write(10,902) (rx(j,i),rho4pir2(j,i),j=1,nxz(i))
      open(10,file=&
      trim(outdir)//trim(compound)//'.QvsR.'//id_at(i),status='replace')
      write(10,902) (rx(j,i),QvsR(j,i),j=1,nx(i))
      close(10)
    endif
    if(RhoPot(2:2)=='y')then
      open(10,file=&
      trim(outdir)//trim(compound)//'.VC.'//id_at(i),status='replace')
      do k=1,nx(i)
...
!-----------------------------------------------------------------------
 ...
 !hf and eeasisss share directory './' for chgden files.
 do ir=1,nieq
   atm=Elem(int(z(ir)))
   open(50,file=trim(atdir)//'chgden'//trim(atm),status='old')
   read(50,*)
...
!      http:/http://www.icts.hkbu.edu.hk/vanhove/
 ...
 implicit none
 integer :: ir,i,imax(nieq)
 real(dp) :: atrho4pi(nxx,nieq),rho4pi(nxx,nieq),&
 ...
 do ir=1,nieq
   ...
   if(abs(cls(ir))>1.d-04) VC(1:nxz(ir),ir)=VC(1:nxz(ir),ir)+cls(ir)
 enddo
 !
 !POTENTIAL AFTER Van H (not updated).
 !call NeighborShells
 ...
 if(RhoPot(1:1)=='y')then
   do ir=1,nieq
     open(10,file=&
     trim(outdir)//trim(compound)//'.RHO4pir2.'//id_at(ir),status='replace')
     write(10,902) (rx(i,ir),rho4pir2(i,ir),i=1,nxz(ir))
     open(10,file=&
     trim(outdir)//trim(compound)//'.QvsR.'//id_at(ir),status='replace')
 ...
 if(RhoPot(2:2)=='y')then
   do ir=1,nieq
     open(10,file=&
     trim(outdir)//trim(compound)//'.VC.'//id_at(ir),status='replace')
     do i=1,nxz(ir)
...
real(dp) :: rho4pi(nxx,nieq),atrho4pi(nxx,nieq),x1,x2,a
...
return
  ...
  !--------------------------------------------------------------------
  real(dp) function V0interstitial(Etmp,xtmp)
  !Ref.: L.Hedin and B.I.Lundqvist, J.Phys.C 4, 2064 (1971), eq. (3.6).
  use param,only : fpi3,facmy
  use maths,only : interpol_egl3
  use space,only : nieq,neq,volXC,nxz
  implicit none
  real(dp),intent(in) :: Etmp,xtmp(nieq)
  integer :: i
  real(dp),parameter :: thrd=1.d0/3.d0
  real(dp) :: myxc,vMT,myx,xs,beta,zUC,eMT
  logical :: prnt
  !
  !rsis interstitial.
  ...
  myxc=-beta*myx
  psave=0.d0
  V0interstitial=xc_myxc(Etmp,rsis,myxc)*myxc
  ...
  do ir=1,nieq
    psave=0.d0
    do i=1,nxz(ir)
      f(i)=xc_myxc(Exc-VCit(i,ir),rs(i,ir),myxc(i,ir))
    enddo
  ...
  !-------------------------------------------------------------------
  real(dp) function xc_myxc(Etmp,rs,myxc)
  ...
  implicit none
  integer :: j
  real(dp)  :: se1,Etmp,rs,myxc,pF,p1,p2,pFm2
  real(dp),parameter :: pi9_4thrd=1.919158292677504d0
  pF=pi9_4thrd/rs
  pFm2=1.d0/(pF*pF)
  se1=self_energy(1.d0,rs)
  !rough approximation.
  if(abs(psave)<1.d-14)then
    p1=1.d0+Etmp*pFm2
  else
    p1=1.d0+(Etmp+(1.d0-self_energy(psave,rs)/se1)*myxc)*pFm2
  endif
  if(p1<0.d0)then
    p1=0.d0
    goto 1
  ...
  !---------------------------------------------------------------------
  real(dp) function self_energy(ps,rs)
  ...
  !---------------------------------------------------------------------
  real(dp) function extrapol_tl(ps,rs)
  !EXTRAPOL_TL=inverse-wavenumber extrapolation from given Vxc tables.
  implicit none
  real(dp),parameter :: ps1=2.80d0,ps2=3.00d0
  real(dp) :: ps,rs,v1,v2,g,b,a
  ...
  !---------------------------------------------------------------------
  real(dp) function interpol_tl(ps,rs)
  ...
  real(dp)  :: ps,rs,a1,a2,a3,p21,r21,s,ta,a,b
  ...
  !---------------------------------------------------------------------
  real(dp) function phi(x,y,xa,ya,xb,yb,ta)
  real(dp) :: x,y,xa,ya,xb,yb,ta
 ...
 !when executed many times, it converges to the normal distribution.
  ...
  implicit none
  integer :: i,m
  real(dp) :: X(3),fitness,errv,x5,tmp
  m=minloc(v0ev,1)
  x5=v0ev(1)
  errv=0.d0
  do i=1,m
 ...
 !--------------------------------------------------------------------
  ...
  implicit none
  integer :: i
  real(dp)  :: y(2),dydx(2),x,p,vmv0x,qbx,emv
...
contains
  ...
  real(dp) :: Exc,Vrmt,x1,x2,xn,d,dudx,dvdx,a,r(nieq,nieq),&
    x(n2),u(n2),v(n2),xk
  !
  if(SelMTrad=='M')then
  ...
  use param,only : SelVMadl,Elem,rydb,compound,RhoPot,id_at,atdir
  ...
  use enrgy,only : ie,ne,MTxc,V0interstitial,VC,VCit,Vtot,QvsR,E_DE,Vxc,z,eev
  use enrgy,only : wrMadl,dVeV,rsis,V01,Vtot1
  use enrgy,only : V0,Vstp,mVstp,Vrmt,rerr,verr
  implicit none
  integer :: i,j,iter,rl
  real(dp) :: VM(nieq),Einc,Exc,a,gNN(nieq),fitness
  real(dp) :: XCmin(nieq),XCmax(nieq),bestmem_XC(nieq),qmt(nieq)
  character(len=255) :: fil
  !
  if(SelVMadl=='y'.and.ie==1)then
  !if(SelVMadl=='y')then
...
DO ITER=1,6
  ...
  !last fitness value is generally not the lowest value.
      ...
      !errMT calculates  V0,Vstp,mVstp,Vrmt,rerr(B),verr(eV)
  ...
  if(SelVMadl=='y')then
    ...
    wrMadl=.false.
    if(ie==1.and.iter==1) wrMadl=.true.
  ...
  write(61,906)'Vstp (eV)',Vstp(1:nieq)*rydb
  write(61,909)'in eV: V0=',V0*rydb,', mean[Vstp]=',mVstp*rydb,', verr=',verr
  write(61,909)'|mean[Vstp]|<=',dVeV
...
ENDDO !ITER
  ...
  !rMT print-out.
  fil=trim(outdir)//trim(compound)//'.rMT'
  open(65,file=trim(fil),status='unknown')
  write(65,'(f8.4,20f7.4)') eev(ie),(rmt(i),i=1,nieq)

  if(SelVMadl=='n')then
    !qMT write(63 to atdir.
    if(ie==1)then
      fil=trim(atdir)//trim(compound)//'.qMT'
      open(63,file=trim(fil),status='unknown')
    endif
    write(63,'(f8.4,20f8.4)') eev(ie),(qmt(i),i=1,nieq)
  elseif(SelVMadl=='y')then
    !qMT  read(63 from atdir in subroutine ProcRoySoc_ElectroStaticMatrix.
    !qMT write(64 to outdir.
    if(ie==1)then
      fil=trim(outdir)//trim(compound)//'.qMT'
      open(64,file=trim(fil),status='unknown')
    endif
    write(64,'(f8.4,20f8.4)') eev(ie),(qmt(i),i=1,nieq)
  endif

  !Vtot and Vxc print-out.
  if(ie==1.and.RhoPot(2:2)=='y')then
    fil=trim(outdir)//trim(compound)
    do i=1,nieq
      open(10,file=trim(fil)//'.Vtot.'//id_at(i),status='replace')
      open(11,file=trim(fil)//'.Vxc.'//id_at(i),status='replace')
      do j=1,nxz(i)
  ...
  if(ie==ne)then
    ...
    write(61,'(a,i0,1x,i0)')'rmt: iolength*4= & nieq*nev*8= ',&
                                  rl*4,nieq*ne*8
    open(62,file=trim(fil)//'.tm_rmt',form='unformatted',recl=rl*4)
    write(62) rmt1
    close(62)
    !
    ...
    write(61,'(a,i0,1x,i0)')'V0: iolength*4= & nev*8= ',&
                                 rl*4,ne*8
    open(62,file=trim(fil)//'.tm_V0',form='unformatted',recl=rl*4)
    write(62) V01
    close(62)
    !
    ...
    write(61,'(a,i0,1x,i0)')'Vtot: iolength*4= & n0*nieq*nev*8= ',&
                                   rl*4,nxx*nieq*ne*8
    open(62,file=trim(fil)//'.tm_Vtot',form='unformatted',recl=rl*4)
    write(62) Vtot1
    close(62)
  endif
  return
  ...
  implicit none
  integer :: ir
  real(dp) :: harvest,fitness
  real(dp) :: X(1:Dim_XC),VM(nieq),dmt2,gNN(nieq),Exc
  real(dp),parameter :: Excn=60.d0/13.60569172d0
  intrinsic random_number
  rerr=0.d0; verr=0.d0
  ...
  use enrgy,only : ie,QvsR,z,ewii,dpii,wrMadl
  implicit none
  integer :: i
  real(dp) :: X(nieq),VM(nieq),zmt(nieq),qewii(nieq),qdpii(nieq),&
    a,zmtm(nieq)
  ...
  use param,only : pi,fpi3,compound,atdir
  use maths,only : interpol_egl3
  use space,only : nieq,neq,ieq,nlatp,rcX,rk,volXC
  use enrgy,only : ie,ewii,dpii,z,wrMadl
  implicit none
  real(dp),parameter :: thrd=1.d0/3.d0,ewerr=1.d-013
  integer,parameter :: np2=20,nbeta=4
  !
  integer :: i,i1,i2,i3,j,n,np,npc,nij,n1,n2,k,nc(np2),l,m
  real(dp) :: rc(3,3),gc(3,3),p(3),dpm(3),tbsqpi,p2,b2,erfc,&
    facg,tpi,a,radAT,r,sc,rr,rg,amin,co,dfac,JMS
  real(dp) :: zmt(nieq),tmp(nieq),ql(nlatp),d(3,nlatp,nlatp),&
  ...
  enddo
  !MT charges zmt fetched from a previous option SelVMadl='n'.
  if(ie==1)then
    fil=trim(atdir)//trim(compound)//'.qMT'
    open(63,file=trim(fil),err=1,status='old')
    allocate(ewii(nieq,nieq),dpii(nieq,nieq))
    goto 2
    1 continue
      write(61,'(3a)')'file ',trim(fil),' not available, stop'
      stop
  endif
  2 continue
    read(63,*) a,(tmp(i),i=1,nieq)
    zmt=tmp-z
  ...
  nij=nlatp*(nlatp+1)/2
  do k=1,nbeta
  beta(k)=1.d0/(radAT*2**(k+1))  !+1 istf -1
  ...
  if(k==nbeta-1.and.wrMadl)then
    open(40,file=trim(outdir)//trim(compound)//'.VM.ewll',status='replace')
  ...
  !EWALD MATRIX nlatp:nieq.
    ...
    enddo
  if(k==nbeta-1.and.wrMadl)then
    open(40,file=trim(outdir)//trim(compound)//'.VM.ewli',status='replace')
  ...
  enddo
  if(k==nbeta-1.and.wrMadl)then
    open(40,file=trim(outdir)//trim(compound)//'.VM.ewii',status='replace')
  ...
  !***Hartree atomic units.***
  if(wrMadl)then
    open(40,file=trim(outdir)//trim(compound)//'.VM.dpll',status='replace')
  ...
  enddo
  if(wrMadl)then
    open(40,file=trim(outdir)//trim(compound)//'.VM.dpli',status='replace')
  ...
  enddo
  if(wrMadl)then
    open(40,file=trim(outdir)//trim(compound)//'.VM.dpii',status='replace')
...
contains
  ...
  real(dp) :: V0tab(ne),avrmt(nieq),v0c(9),bj(0:lmax+1),by(0:lmax+1)
  real(dp) :: psr(ne,0:lmax,nieq),psrm(ne,0:lmax,nieq),psrp(ne,0:lmax,nieq)
  real(dp) :: qr,volmt,bohr3,Einc,Exc,avverr,avrerr
  real(dp),parameter :: Excn=60.d0/13.60569172d0
  bohr3=bohr**3
...
928 format('MT SpaceFill=',f0.3)
  ...
  real(dp)  :: delta(ne,0:lmax,nieq),bj(0:lmax+1),by(0:lmax+1),y(2),&
...
!    if(ie==ne.and.tag/='+')then
      ...
      open(10,file=&
      trim(outdir)//trim(compound)//'.UvsR.'//idl(1:j)//id_at(ir))
      do i=1,nox
...
enddo
  ...
  real(dp) :: t,y(2),ysta(2),tout,t_end,qr,or(node),ou(2,node),w_max(2),&
  ...
  real(dp)  :: t1,ysta(2),bsl(0:lmax+1),alfaz,gamma,z0,a0,b0,v0,&
    gg,kg,ee
...
if(l<lsta)then
  ...
  implicit none
  integer :: ir,l,npi,i
  real(dp)  :: psrm(ne,0:lmax,nieq),psrp(ne,0:lmax,nieq),del(ne),dif,pih
pih=pi*0.5d0
do ir=1,nieq
  do l=0,lmax
...
enddo !ir
  ...
  use param,only : outdir,compound,rydb
  ...
  implicit none
  integer :: i,m
  real(dp) :: V0tab(ne),v0c(9),a,dev1,dev2,v0tmp(ne),tmp
  real(dp),allocatable :: XCmin(:),XCmax(:),bestmem_XC(:)
  character(len=255) :: fil
 allocate(v0ev(ne))
 v0ev=V0tab*rydb
 fil=trim(outdir)//trim(compound)//'.V0vsE'
 open(60,file=trim(fil),status='replace')
 !
 ! constant inner potential.
 if(abs(v0ev(1)-v0ev(ne))<1.d-06)then
 ...
 !
 !do V0vsE_analytic.
 open(60,file=trim(fil)//'_analytic',status='replace')
 ...
 904 format('iter,misfit= ',i4,f6.2,' eV')
  ...
  integer :: i,ir,l
  real(dp) :: delta(ne,0:lmax,nieq),V0tab(ne),emv0(ne)
  ...
  do ir=1,nieq
    ...
    if(SpinPS=='y'.and.tag=='+')then
      pid=trim(compound)//'.PS.su.'//id_at(ir)
    elseif(SpinPS=='y'.and.tag=='-')then
      pid=trim(compound)//'.PS.sd.'//id_at(ir)
    else
      pid=trim(compound)//'.PS.sl.'//id_at(ir)
    endif
    open(40,file=trim(outdir)//trim(pid),status='replace')
!!!    write(40,900) lmax+1,trim(txt)//trim(txu) !identification line.
    do i=1,ne
  ...
  integer :: i,j,l,ir,itheta,lxq(ne,nieq)
  real(dp) :: psrm(ne,0:lmax,nieq),&
    psrp(ne,0:lmax,nieq),facr(ne),s(ne),dsdo(0:ntheta,ne),&
    shf(0:ntheta,ne),V0tab(ne),dtheta,emv0,tpi,fpi,frac,fg,rf,rg
  real(dp) :: p(0:ntheta,0:lmax,0:1)
  complex(dp) :: cf(0:lmax),cg(0:lmax)
  complex(dp) :: f,g,cm,cp
...
do ir=1,nieq
  ...
  do i=1,ne
    ...
    do l=lxq(i,ir),0,-1 !sum from small to large phase shifts.
      ...
      cf(l)=(l+1)*cm+l*cp
      cg(l)=cp-cm
      rf=real(cf(l)*conjg(cf(l)),dp)
      rg=real(cg(l)*conjg(cg(l)),dp)
      s(i)=s(i)+(rf+(l*l+l)*rg)/(l+l+1)
    enddo
    s(i)=s(i)*facr(i)
    !
    do itheta=0,ntheta
      f=cmplx(0.d0,0.d0,dp)
      g=cmplx(0.d0,0.d0,dp)
      do l=lxq(i,ir),0,-1 !sum from small to large phase shifts.
        f=f+cf(l)*p(itheta,l,0)
        g=g+cg(l)*p(itheta,l,1)
      enddo
      fg=real(f*conjg(f)+g*conjg(g),dp)
      dsdo(itheta,i)=fg*facr(i)
      shf(itheta,i)=real(ci*(f*conjg(g)-conjg(f)*g)/fg,dp)
    enddo !itheta
  enddo !i
  !
  if(CrossSection=='y')then
    open(10,file=trim(fil)//'.sigma.'//id_at(ir),status='replace')
    do i=1,ne
  ...
  do i=1,ne
    ...
    j=len_trim(id_e)
    if(Omega=='y')then
      open(10,file=trim(fil)//'.dsdo.'//id_e(1:j)//id_at(ir),status='replace')
      do itheta=0,ntheta
    ...
    if(Theta=='y')then
      open(10,file=trim(fil)//'.dsdt.'//id_e(1:j)//id_at(ir),status='replace')
      do itheta=0,ntheta
    ...
    if(Sherman=='y')then
      open(10,file=trim(fil)//'.shf.'//id_e(1:j)//id_at(ir),status='replace')
      do itheta=0,ntheta
...
!=============================================================================
 ...
 !             Phys.Rev.B76,195441(2007).
 !
 use param,only : outdir,atdir,SelVCoul
 use enrgy,only : WIEN2kData,FreeAtomData,superpose
 use calPS,only : PartialWaveMethod
 implicit none
 integer :: time(8)
 character(len=*), intent(in) :: input_file,log_file,output_dir,atom_dir
 ...
 write(61,'(3a)') 'log ',trim(outdir)//trim(log_file),' begins.'
 !
 !timing.
 ...
 call ModesOfCalc(trim(input_file))
 call Structure(trim(input_file))
 call NeighborShells
 call EnergyGrid
 ...
 !---------------------------------------------------------------------
  subroutine ModesOfCalc(inputf)
  use param,only : SelVxc,SelMTrad,SelVMadl,SpinPS,CrossSection,SelVCoul
  ...
  real(dp) :: UOL,rcc1,rcc2,rcc3,rc(3,3)
 ...
 read(5,*) txt
    write(61,'(a)') txt(1:10)
 read(5,'(a)') compound
    compound=adjustl(trim(compound))
...
real(dp) :: rc(3,3),rj(3),dr,r,a,b
real(dp),parameter :: rmax=15.d0
!
rc=rcX
...
!---------------------------------------------------------------------
  ...
  !ENERGY GRID can be uniform or non-uniform (by keeping or remowing !).
  use param,only : dp
  use space,only : nieq,rmt1
  use enrgy,only : ne,eev,ee1,ee2,dee,V01
  implicit none
  integer :: je
  real(dp) :: E,egrid(1000)
  je=0; E=ee1-dee
  1 continue
     if(E>=ee2) goto 2
     je=je+1
